...
def generate_choropleth(data):
    ...
    ax.set_title('Permanent Resident Count by Country in Trade Area', fontsize=20)
    img_buf = BytesIO()
            ...
            if st.sidebar.title("Digital Consultant"):
                ...
                if st.sidebar.button("Generate Business Idea"):
                    with st.spinner('Generating business idea...'):
                        business_prompt = (
                            f"Based on the following Data Identify which ethnic group should be the best to target and create a unique business idea tailored towards that group."
                ...
                if st.sidebar.button("Generate Marketing Strategy"):
                    with st.spinner('Generating marketing strategy...'):
                        strategy_prompt = (
                                ...
                                "Please format neatly"
                                )

                        strategy = get_openai_insight(strategy_prompt, data_context, st.session_state.user_type)
                ...
                if st.sidebar.button("Identify Best Products/Brands to Launch"):
                    with st.spinner('Identifying best products/brands...'):
                        products_prompt = (
                            f"Analyze the demographic data to recommend 3 existing and real world products or brands that would appeal to the diverse community composition, especially focusing on the larger groups of permanent residents."
                            "Highlight potential products or services that align with the cultural preferences, lifestyle, and consumption habits of these groups."
                            "Also, consider any gaps in the current market offerings that these products or brands could fill. Please list off real world products and brands. and make sure to explain how you used the dataset given. The title and Each product should start on a new line."
                            "Use external information or the web to ensure that each answer contains real world brands and or products."
                            "Please format neatly"
                        )
                        products = get_openai_insight(products_prompt, data_context, st.session_state.user_type)
                        st.write("Best Products/Brands:\n", products)
                        # Save to PDF
                ...
                if st.sidebar.button("Suggest CSR Initiatives"):
                    with st.spinner('Suggesting CSR initiatives...'):
                        ...
                        if 'generated_idea' in st.session_state:
                            csr_idea = st.session_state.generated_idea

                            # Construct the prompt with the previously generated idea
                            csr_prompt = (
                                f"Alongside the previously generated idea: '{csr_idea}', using the demographic outlined in the spreadsheet, "
                                "write out one CSR Initiative that a Canadian company can implement using the data provided along with external research. "
                                "The first Initiative which relates to the demographic should be creative, unique, and interesting. "
                                "The second Initiative related to the demographic should tackle a serious problem within that country's demographic. "
                                "The third initiative related to the demographics should be a unique, funny, comedic, and interesting idea. "
                                "Each initiative should start on a new line and be uniquely tailored to that specific country in terms of their cultural habits."
                            )

                            # Generate the CSR initiatives
                            csr = get_openai_insight(csr_prompt, data_context, st.session_state.user_type)
                            st.write("CSR Initiatives:\n", csr)
                            ...
                            csr_prompt1 = (
                                f"using the 3 demographic outlined in the spreadsheet, "
                                "Be specific and but also please be mindful and culturally sensitive"
                                "write out one CSR Initiative that a Canadian company can implement using the data provided along with external research. "
                                "The first Initiative which relates to the demographic should be creative, unique, and interesting. "
                                "The second Initiative related to the demographic should tackle a serious problem within that country's demographic. "
                                "The third initiative related to the demographics should be a unique, funny, comedic, and interesting idea. "
                            ...
                            # Generate the CSR initiatives
                            csr = get_openai_insight(csr_prompt1, data_context, st.session_state.user_type)
                            st.write("CSR Initiatives: \n", csr)
                            # Save to PDF
